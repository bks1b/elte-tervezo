...
export default cache(
  CACHE_KEY,
  () =>
    fetch(SHEET_URL).then(res => res.text()).then(res =>
      fetch(SHEET_URL + res.match(/".+?(\/.+?\.xlsx)"/)![1])
      ...
      const result = {
        ...
        filterOptions: {
          specs: {} as Dict<number>,
          optionalities: new Set<string>(),
          maxSemester: 0,
        },
      };
      for (
        const row of utils.sheet_to_json(Object.values(read(buf).Sheets)[0], {
          range: 2,
          header: 1,
          raw: true,
        }) as string[][]
        ...
        const courseSpec = [0, 1].reduce((m, i) =>
          m
            ?? note.match(
              new RegExp(
