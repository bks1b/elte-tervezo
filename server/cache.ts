...
async (overwrite = false): Promise<T> => {
  ...
  const cached = !overwrite && await cache.get(key);
  if (typeof cached === 'string') {
    return cached
      ? JSON.parse(brotliDecompressSync(Buffer.from(cached, 'base64')).toString('utf8'))
      : undefined as T;
  }
  ...
  const request = f(overwrite).then(res =>
    cache.set(
      key,
      res ? brotliCompressSync(JSON.stringify(res)).toString('base64') : '',
      res ? undefined : { ttl: FAILED_TTL },
    ).then(() => res)
  );
